...
public class SystemLogService {
    ...
    public void write(
        ...
        try {
            ...
            // Table may not exist yet (migrations not run during early startup).
            // Fall back to stderr so startup is not blocked.
            System.err.printf(
                "[SYSTEM_LOG_FALLBACK] %s | %s | %s | correlation=%s | source=%s | payload=%s%n",
                level.name(),
                category.name(),
                event,
    ...
    private static SystemLog mapRow(ResultSet rs, int rowNum)
        throws SQLException {
