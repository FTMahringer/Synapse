...
- `v2.0.0-dev` marks the active implementation track after `v1.0.0`.
- Each implementation slice should be committed as a small versioned step.
- Patch-sized commits use `v2.0.x-dev` in notes and tags only when they represent a validated slice.
- Milestone-sized commits use `v2.x.0-dev` while the milestone is still open.
...
|---|---|---|
| `v2.1.0-dev` | Persistence API Layer | Real CRUD and query APIs over the v1 database foundation |
| `v2.2.0-dev` | Auth and Users | Sign-in, JWT sessions, roles, and role-scoped access |
| `v2.3.0-dev` | Model Providers | Provider registry plus Ollama and external-provider execution |
| `v2.4.0-dev` | Chat Runtime | Conversation creation, message send, model response, persistence |
| `v2.5.0-dev` | Agent Orchestration | Main Agent routing, teams, task dispatch, AI-Firm entry points |
| `v2.6.0-dev` | Realtime Runtime | SSE/WebSocket event bus for logs and conversation events |
| `v2.7.0-dev` | Plugin and Store Runtime | Install, enable, disable, validate, and list plugins/bundles |
| `v2.8.0-dev` | Dashboard Management | Full operator UI over V2 APIs |
| `v2.9.0-dev` | CLI Runtime | Go CLI/TUI connected to backend APIs |
| `v2.10.0-dev` | Release Hardening | Tests, CI, docs, migrations, and `v2.0.0` release readiness |

---

## 6. Detailed Milestones

### v2.1.0-dev - Persistence API Layer

Goal: replace runtime-only DTOs with database-backed services where the schema already exists.

Patch steps:

- `v2.0.1-dev`: Add repository/service boundaries for users, agents, plugins, model providers, conversations, messages, tasks, and settings.
- `v2.0.2-dev`: Add stable DTOs and validation classes for all read/write APIs.
- `v2.0.3-dev`: Implement agents and teams CRUD using database tables while preserving file-defined bootstrap agents.
- `v2.0.4-dev`: Implement settings and system metadata APIs.
- `v2.0.5-dev`: Implement task and task log APIs.
- `v2.0.6-dev`: Add API error model, request correlation IDs, and structured logging for mutating endpoints.
...
### v2.2.0-dev - Auth and Users

Goal: make SYNAPSE multi-user instead of an unauthenticated local console.

Patch steps:

- `v2.1.1-dev`: Add password hashing, user bootstrap, and owner account initialization.
- `v2.1.2-dev`: Add login, refresh, logout, and session revocation APIs.
- `v2.1.3-dev`: Add JWT verification middleware and role annotations.
- `v2.1.4-dev`: Enforce OWNER, ADMIN, USER, and VIEWER permissions on backend APIs.
- `v2.1.5-dev`: Add dashboard login, session storage, logout, and role-aware navigation.
- `v2.1.6-dev`: Add audit logs for auth events without exposing credentials.
...
### v2.3.0-dev - Model Providers

Goal: implement real model calls behind a provider-neutral interface.

Patch steps:

- `v2.2.1-dev`: Add provider configuration model with encrypted secret storage boundary.
- `v2.2.2-dev`: Implement Ollama provider health and model listing.
- `v2.2.3-dev`: Implement Ollama chat completion request path.
- `v2.2.4-dev`: Implement OpenAI-compatible provider path.
- `v2.2.5-dev`: Implement Anthropic provider path.
- `v2.2.6-dev`: Add provider cost and latency logging.
- `v2.2.7-dev`: Add provider test endpoint that does not store prompt content by default.
...
### v2.4.0-dev - Chat Runtime

Goal: make conversations functional end to end.

Patch steps:

- `v2.3.1-dev`: Add conversation create/list/detail APIs.
- `v2.3.2-dev`: Add message send API with persistence.
- `v2.3.3-dev`: Add Main Agent prompt assembly from identity, soul, connections, and system prompt.
- `v2.3.4-dev`: Connect message send to selected model provider.
- `v2.3.5-dev`: Persist assistant responses, token estimates, latency, and provider metadata.
- `v2.3.6-dev`: Add cancellation and timeout handling.
- `v2.3.7-dev`: Add dashboard conversation view with real messages.
...
### v2.5.0-dev - Agent Orchestration

Goal: implement the SYNAPSE agent model beyond static file listing.

Patch steps:

- `v2.4.1-dev`: Add agent runtime registry and activation states.
- `v2.4.2-dev`: Add Main Agent router service with deterministic routing rules.
- `v2.4.3-dev`: Add team leader/member dispatch contract.
- `v2.4.4-dev`: Add AI-Firm project/task dispatch entry point.
- `v2.4.5-dev`: Add agent memory vault read/write hooks.
- `v2.4.6-dev`: Add heartbeat records for active agents.
- `v2.4.7-dev`: Add dashboard management for agent activation and routing inspection.
...
### v2.6.0-dev - Realtime Runtime

Goal: stream platform events instead of relying only on polling.

Patch steps:

- `v2.5.1-dev`: Add internal event publisher abstraction.
- `v2.5.2-dev`: Add Redis Streams-backed log fanout.
- `v2.5.3-dev`: Add SSE endpoint for live logs.
- `v2.5.4-dev`: Add WebSocket endpoint for conversation events.
- `v2.5.5-dev`: Add dashboard live log panel.
- `v2.5.6-dev`: Add dashboard streaming conversation updates.
- `v2.5.7-dev`: Add reconnect, backoff, and polling fallback behavior.
...
### v2.7.0-dev - Plugin and Store Runtime

Goal: make plugins and store entries installable resources.

Patch steps:

- `v2.6.1-dev`: Add manifest parser and validator for channels, models, skills, and MCP plugins.
- `v2.6.2-dev`: Add plugin install/enable/disable/uninstall APIs.
- `v2.6.3-dev`: Add store registry sync from local `store/registry.yml`.
- `v2.6.4-dev`: Add bundle validation and install flow.
- `v2.6.5-dev`: Add plugin stats tracking.
- `v2.6.6-dev`: Add dashboard plugin/store management views.
- `v2.6.7-dev`: Add safety rules for community plugin install prompts.
...
### v2.8.0-dev - Dashboard Management

Goal: turn the dashboard from status panels into the primary operator interface.

Patch steps:

- `v2.7.1-dev`: Add routing, state management, and API error handling structure.
- `v2.7.2-dev`: Add authenticated layout and role-aware navigation.
- `v2.7.3-dev`: Add providers screen.
- `v2.7.4-dev`: Add conversations screen.
- `v2.7.5-dev`: Add agents and teams screen.
- `v2.7.6-dev`: Add plugins and store screens.
- `v2.7.7-dev`: Add logs and observability screen.
- `v2.7.8-dev`: Add settings and user management screens.
...
### v2.9.0-dev - CLI Runtime

Goal: implement the Go CLI described in the v0 docs.

Patch steps:

- `v2.8.1-dev`: Add Go module and command shell.
- `v2.8.2-dev`: Add auth login/logout/session commands.
- `v2.8.3-dev`: Add health, logs, agents, providers, and plugin commands.
- `v2.8.4-dev`: Add conversation send and stream commands.
- `v2.8.5-dev`: Add Bubble Tea TUI overview.
- `v2.8.6-dev`: Add config profile handling.
...
### v2.10.0-dev - Release Hardening

Goal: make `v2.0.0` safe to tag as the first operational platform release.

Patch steps:

- `v2.9.1-dev`: Add backend unit and integration tests for auth, providers, chat, logs, and plugins.
- `v2.9.2-dev`: Add frontend test/build CI.
- `v2.9.3-dev`: Add migration validation CI.
- `v2.9.4-dev`: Add Compose smoke test workflow.
- `v2.9.5-dev`: Update docs to match implemented behavior.
- `v2.9.6-dev`: Add release notes with all V2 changelog entries.
- `v2.9.7-dev`: Tag and release `v2.0.0`.
